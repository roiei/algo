...
and a sorted list releaseTimes, where releaseTimes[i] 
...
The tester wants to know the key of the keypress 
that had the longest duration. 
...
class Solution:
    def slowestKey(self, releaseTimes: [int], keysPressed: str) -> str:
        ...
        for rel, key in zip(releaseTimes, keysPressed):
            if mx < rel - prev:
                mx = rel - prev
                mx_key = [key]
            elif mx == rel - prev:
...
stime = time.time()
print("c" == Solution().slowestKey(releaseTimes = [9,29,49,50], keysPressed = "cbcd"))
