
import time
from util.util_list import *
from util.util_tree import *
import copy
import collections


"""
A sequence of numbers is called arithmetic 
if it consists of at least two elements, 
and the difference between every two consecutive elements is the same. 
More formally, 
a sequence s is arithmetic if and only if s[i+1] - s[i] == s[1] - s[0] for all valid i.

For example, these are arithmetic sequences:
The following sequence is not arithmetic:

1, 1, 2, 5, 7
You are given an array of n integers, nums, 
and two arrays of m integers each, l and r, 
representing the m range queries, where the ith query is the range 
[l[i], r[i]]. All the arrays are 0-indexed.

Return a list of boolean elements answer, 
where answer[i] is true if the subarray nums[l[i]], nums[l[i]+1], ... , nums[r[i]] can be rearranged to form an arithmetic sequence, and false otherwise.

In the 0th query, the subarray is [4,6,5]. This can be rearranged as [6,5,4], which is an arithmetic sequence.
In the 1st query, the subarray is [4,6,5,9]. This cannot be rearranged as an arithmetic sequence.
In the 2nd query, the subarray is [5,9,3,7]. This can be rearranged as [3,5,7,9], which is an arithmetic sequence.
"""


class Solution:
    def checkArithmeticSubarrays(self, nums: [int], l: [int], r: [int]) -> [bool]:
        def is_arithmetic(nums):
            if not nums or len(nums) < 2:
                return False

            n = len(nums)
            diff = nums[1] - nums[0]
            for i in range(2, n):
                if nums[i] - nums[i - 1] != diff:
                    return False
            return True

        res = []
        for left, right in zip(l, r):
            wnd = nums[left:right + 1]
            if is_arithmetic(sorted(wnd)) or is_arithmetic(sorted(wnd, reverse=True)):
                res += True,
                continue
            res += False,

        return res


stime = time.time()
print([True,False,True] == Solution().checkArithmeticSubarrays(\
    nums = [4,6,5,9,3,7], l = [0,0,2], r = [2,3,5]))
print('elapse time: {} sec'.format(time.time() - stime))
